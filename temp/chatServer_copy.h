...
class chatServer {
    ...
    public:
        ...
        virtual void WorkerThread(HANDLE iocp) = 0;

        // AssociateSocket - 
        virtual void AssociateSocket(SOCKET clientSocket) = 0;
        ...
        virtual void handleSelectConnections() = 0;
...
struct chatServerFactory {
    inline static chatServer& getInstance(Logger &logger,
                                            const string& serverName,
                                            const string& portNumber) {
#ifdef _WIN32
        static instance(logger, serverName, portNumber);
#else
